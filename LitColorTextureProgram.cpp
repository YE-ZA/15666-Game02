...
LitColorTextureProgram::LitColorTextureProgram() {
	...
	,
		...
		"	float fresnel = 4.0f * clamp(pow((1 - abs(dot(n, CAMERA_DIRECTION))),3), 0.1, 1);\n"
		"	fragColor = vec4(e*albedo.rgb * 2.5f * fresnel * clamp(sin(position/(0.9 + abs(sin((TIME+10)*0.1)))), 0.5, 1), albedo.a);\n"
		"}\n"
	);
	...
	CAMERA_DIRECTION_vec3 = glGetUniformLocation(program, "CAMERA_DIRECTION");
